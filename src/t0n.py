...
class t0n:
    ...
    def getVNew(self, vOld: np.ndarray, xi: np.ndarray, muNew: np.ndarray, ai: float): #18
        return vOld - ai * (np.dot(xi,muNew) + ai)/(np.dot(xi,np.multiply(vOld,xi)) +1.0) *  np.multiply(np.multiply(vOld, xi),np.multiply(vOld,xi))
    ...
    def getSi(self, z:float, viNew: np.ndarray, vOld: np.ndarray, miNew: np.ndarray, muOld: np.ndarray) -> float:#21
        #not sure about this
        return norm.cdf(z) * np.prod(np.sqrt(np.divide(viNew + vOld, viNew))) * np.exp(np.sum(np.divide(np.power(miNew - muOld,2), 2.0 *(vOld + viNew))))
